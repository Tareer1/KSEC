...
from ksec.cli import module as module_commands
from ksec.cli import purple as purple_commands
from ksec.cli import change as change_commands
from ksec.cli import history as history_commands
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_upass = u_sub.add_parser("password", help="Reset a user's password", parents=[common_sub])
    p_upass.add_argument("username")
    p_upass.add_argument("--password", default=None, help="New password (prompt/generate if omitted)")
    p_upass.add_argument("--actor", default="", help="Audit actor (defaults to the target username)")
    p_upass.set_defaults(func=admin_commands.cmd_user_password)
    ...
    for name, func in (
        ...
        p.add_argument("id")
        p.set_defaults(func=func)
    j_logs = j_sub.add_parser("logs", help="Show a job's captured stdout/stderr", parents=[common_sub])
    j_logs.add_argument("id")
    j_logs.set_defaults(func=jobs_commands.cmd_job_logs)
    j_retry = j_sub.add_parser("retry", help="Resubmit a terminal job as a fresh job", parents=[common_sub])
    j_retry.add_argument("id")
    j_retry.set_defaults(func=jobs_commands.cmd_job_retry)
    j_trace = j_sub.add_parser("trace", help="Trace a job's session/schedule/audit lineage", parents=[common_sub])
    j_trace.add_argument("id")
    j_trace.set_defaults(func=jobs_commands.cmd_job_trace)
    j_health = j_sub.add_parser("health", help="Show live scheduler health", parents=[common_sub])
    j_health.set_defaults(func=jobs_commands.cmd_job_health)
    ...
    r_create.add_argument("--format", default="markdown", choices=["markdown", "html", "pdf", "docx"])
    r_create.add_argument("--out", default=None, help="Write report to a file (.pdf writes PDF)")
    r_create.add_argument("--user", default="")
    r_create.set_defaults(func=report_commands.cmd_report_create)
    r_preview = r_sub.add_parser("preview", help="Render a report without storing it", parents=[common_sub])
    r_preview.add_argument("--engagement", type=int, default=None)
    r_preview.add_argument("--title", default=None)
    r_preview.add_argument("--format", default="markdown", choices=["markdown", "html", "pdf", "docx"])
    r_preview.set_defaults(func=report_commands.cmd_report_preview)
    r_export = r_sub.add_parser("export", help="Export a stored report as PDF or DOCX", parents=[common_sub])
    r_export.add_argument("id", type=int)
    r_export.add_argument("--format", default="pdf", choices=["pdf", "docx"])
    r_export.add_argument("--out", default=None, help="Output path (default report-<id>.pdf|.docx)")
    r_export.set_defaults(func=report_commands.cmd_report_export)
    ...
    l_practice = l_sub.add_parser("practice", help="Hands-on practice drills", parents=[common_sub])
    lp_sub = l_practice.add_subparsers(dest="practice_command", metavar="PRACTICE_COMMAND")
    lp_list = lp_sub.add_parser("list", help="List practice drills", parents=[common_sub])
    lp_list.add_argument("--user", default=None)
    lp_list.add_argument("--password", default=None)
    lp_list.set_defaults(func=learn_commands.cmd_learn_practice_list)
    lp_start = lp_sub.add_parser("start", help="Record an attempt on a drill", parents=[common_sub])
    lp_start.add_argument("--id", required=True)
    lp_start.add_argument("--user", required=True)
    lp_start.add_argument("--password", default=None)
    lp_start.set_defaults(func=learn_commands.cmd_learn_practice_start)
    lp_pass = lp_sub.add_parser("pass", help="Mark a drill passed", parents=[common_sub])
    lp_pass.add_argument("--id", required=True)
    lp_pass.add_argument("--user", required=True)
    lp_pass.add_argument("--password", default=None)
    lp_pass.set_defaults(func=learn_commands.cmd_learn_practice_pass)
    ...
    # event-driven workflow triggers (spec 07: beyond cron)
    w_trig = w_sub.add_parser("trigger", help="Event-driven workflow triggers", parents=[common_sub])
    tg_sub = w_trig.add_subparsers(dest="trigger_command", metavar="TRIGGER_COMMAND")
    tg_add = tg_sub.add_parser("add", help="Create a trigger (event_type -> workflow)", parents=[common_sub])
    tg_add.add_argument("--name", required=True)
    tg_add.add_argument("--event-type", required=True, help="e.g. soc.alert.critical | job.failed")
    tg_add.add_argument("--workflow", required=True, help="Workflow or capability to run")
    tg_add.add_argument("--event-glob", default="*", help="Glob on the payload target")
    tg_add.add_argument("--target-field", default="target", help="Payload field holding the target")
    tg_add.add_argument("--workspace", default="RED_TEAM", choices=["RED_TEAM", "BLUE_TEAM", "RESEARCH_OSINT", "ADVERSARY_SIMULATION", "LEARN_WORK"])
    tg_add.add_argument("--user", default="")
    tg_add.set_defaults(func=workflow_commands.cmd_workflow_trigger_add)
    tg_list = tg_sub.add_parser("list", help="List triggers", parents=[common_sub])
    tg_list.set_defaults(func=workflow_commands.cmd_workflow_trigger_list)
    tg_rm = tg_sub.add_parser("remove", help="Remove a trigger", parents=[common_sub])
    tg_rm.add_argument("id", type=int)
    tg_rm.set_defaults(func=workflow_commands.cmd_workflow_trigger_remove)
    tg_on = tg_sub.add_parser("enable", help="Enable a trigger", parents=[common_sub])
    tg_on.add_argument("id", type=int)
    tg_on.set_defaults(func=workflow_commands.cmd_workflow_trigger_enable)
    tg_off = tg_sub.add_parser("disable", help="Disable a trigger", parents=[common_sub])
    tg_off.add_argument("id", type=int)
    tg_off.set_defaults(func=workflow_commands.cmd_workflow_trigger_disable)
    tg_fire = tg_sub.add_parser("fire", help="Fire an event through matching triggers (policy-checked)", parents=[common_sub])
    tg_fire.add_argument("--event-type", required=True)
    tg_fire.add_argument("--target", default=None, help="Target value for matching (falls back to payload.target)")
    tg_fire.add_argument("--payload", default=None, help="JSON object of event fields")
    tg_fire.add_argument("--engagement", type=int, default=None)
    tg_fire.add_argument("--user", required=True)
    tg_fire.add_argument("--password", default=None)
    tg_fire.add_argument("--role", default=None)
    tg_fire.set_defaults(func=workflow_commands.cmd_workflow_trigger_fire)
    ...
    # domain modules (spec 08 #23-27: API, wireless, cloud, container, k8s)
    p_module = sub.add_parser("module", help="Domain modules: API/Wireless/Cloud/Container/Kubernetes posture", parents=[common_sub])
    mo_sub = p_module.add_subparsers(dest="module_command", metavar="MODULE_COMMAND")
    mo_list = mo_sub.add_parser("list", help="List domain modules", parents=[common_sub])
    mo_list.set_defaults(func=module_commands.cmd_module_list)
    mo_info = mo_sub.add_parser("info", help="Module details + installed tools", parents=[common_sub])
    mo_info.add_argument("module")
    mo_info.set_defaults(func=module_commands.cmd_module_info)
    mo_check = mo_sub.add_parser("check", help="Run a module's deterministic offline posture checks", parents=[common_sub])
    ...
    pu_sub = p_purple.add_subparsers(dest="purple_command", metavar="PURPLE_COMMAND")
    pu_ex = pu_sub.add_parser("exercise", help="Purple exercise lifecycle", parents=[common_sub])
    pu_ex_sub = pu_ex.add_subparsers(dest="exercise_command", metavar="EXERCISE_COMMAND")
    pu_new = pu_ex_sub.add_parser("new", help="Create a purple exercise", parents=[common_sub])
    pu_new.add_argument("--name", required=True)
    pu_new.add_argument("--description", default=None)
    pu_new.add_argument("--engagement", type=int, default=None)
    pu_new.add_argument("--user", default=None)
    pu_new.set_defaults(func=purple_commands.cmd_purple_exercise_new)
    pu_list = pu_ex_sub.add_parser("list", help="List exercises", parents=[common_sub])
    pu_list.set_defaults(func=purple_commands.cmd_purple_exercise_list)
    pu_start = pu_ex_sub.add_parser("start", help="Start an exercise", parents=[common_sub])
    pu_start.add_argument("id", type=int)
    pu_start.set_defaults(func=purple_commands.cmd_purple_exercise_start)
    pu_complete = pu_ex_sub.add_parser("complete", help="Tally outcomes and complete an exercise", parents=[common_sub])
    pu_complete.add_argument("id", type=int)
    pu_complete.set_defaults(func=purple_commands.cmd_purple_exercise_complete)
    pu_show = pu_ex_sub.add_parser("show", help="Exercise summary with detection coverage", parents=[common_sub])
    pu_show.add_argument("id", type=int)
    pu_show.set_defaults(func=purple_commands.cmd_purple_exercise_show)
    pu_del = pu_ex_sub.add_parser("delete", help="Delete an exercise", parents=[common_sub])
    pu_del.add_argument("id", type=int)
    pu_del.set_defaults(func=purple_commands.cmd_purple_exercise_delete)

    # change detection (spec 08 #59)
    p_change = sub.add_parser("change", help="Change detection: baselines and drift scans", parents=[common_sub])
    ch_sub = p_change.add_subparsers(dest="change_command", metavar="CHANGE_COMMAND")
    ch_base = ch_sub.add_parser("baseline", help="Baseline management", parents=[common_sub])
    ch_base_sub = ch_base.add_subparsers(dest="baseline_command", metavar="BASELINE_COMMAND")
    cb_new = ch_base_sub.add_parser("create", help="Create a baseline snapshot", parents=[common_sub])
    cb_new.add_argument("--name", required=True)
    cb_new.add_argument("--scope", default="assets", choices=["assets", "findings", "jobs", "config"])
    cb_new.add_argument("--target", default="*", help="Scope filter (e.g. an engagement id, or *)")
    cb_new.add_argument("--user", default=None)
    cb_new.set_defaults(func=change_commands.cmd_change_baseline_create)
    cb_list = ch_base_sub.add_parser("list", help="List baselines", parents=[common_sub])
    cb_list.set_defaults(func=change_commands.cmd_change_baseline_list)
    cb_del = ch_base_sub.add_parser("delete", help="Delete a baseline", parents=[common_sub])
    cb_del.add_argument("id", type=int)
    cb_del.set_defaults(func=change_commands.cmd_change_baseline_delete)
    ch_scan = ch_sub.add_parser("scan", help="Scan for drift against a baseline", parents=[common_sub])
    ch_scan.add_argument("id", type=int)
    ch_scan.add_argument("--user", default=None)
    ch_scan.set_defaults(func=change_commands.cmd_change_scan)
    ch_scans = ch_sub.add_parser("scans", help="List scans (optionally per baseline)", parents=[common_sub])
    ch_scans.add_argument("--baseline", type=int, default=None)
    ch_scans.set_defaults(func=change_commands.cmd_change_scan_list)

    # activity views (read-only)
    p_history = sub.add_parser("history", help="Chronological activity timeline", parents=[common_sub])
    p_history.add_argument("--limit", type=int, default=30)
    p_history.set_defaults(func=history_commands.cmd_history)
    p_graph = sub.add_parser("graph", help="Relationship graph (engagements/assets/findings/cases)", parents=[common_sub])
    p_graph.add_argument("--engagement", type=int, default=None)
    p_graph.set_defaults(func=history_commands.cmd_graph)
