...
class WorkflowEngine:
    ...
    def run(
        ...
        *,
        ...
        self.db.execute(
            ...
            " user_id, status, steps_total, created_at) VALUES (?, ?, ?, ?, ?, ?, 'running',"
            " ?, ?)",
            (
                run_id,
                definition.name,
        ...
        try:
            for step in definition.steps:
                ...
                completed = self.scheduler.wait_for(job.id)
    ...
    def runs(self, workflow: str | None = None, limit: int = 20) -> list[sqlite3.Row]:
        ...
        sql = (
            ...
            " steps_completed, created_at, completed_at, error FROM workflow_runs"
        )
        params: list = []
        if workflow:
            sql += " WHERE workflow = ?"
